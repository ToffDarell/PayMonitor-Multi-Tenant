        ...
        <form action="{{ route('superadmin.plans.store') }}" method="POST">
            ...
            <div class="row g-3 mb-3">
                <div class="col-6">
                    ...
                    <input type="number" name="max_branches" value="{{ old('max_branches', 1) }}" min="1" class="form-control @error('max_branches') is-invalid @enderror" required>
                    @error('max_branches')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-6">
                    <label class="form-label fw-semibold">Max Users</label>
                    <input type="number" name="max_users" value="{{ old('max_users', 5) }}" min="1" class="form-control @error('max_users') is-invalid @enderror" required>
